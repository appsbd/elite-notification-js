<template>
  ...
  <div v-for="(option,key) in options" :key="key" :name="field_name" :type="this.$attrs?.type?this.$attrs.type:'radio'" :value="option.val" >
    ...
    <label :for="field_name+key" :class="(isInline?'apbd-imgr-inline ':'') + optionClass">
      ...
      <slot name="icon_image" :option="option">
        ...
        <slot name="label" :option="option">
          <slot :name="'label-'+option.val" :option="option">
            <translate class="apbd-imgr-option-label" v-if="option?.label">{{ option.label }}</translate>
          </slot>
        </slot>
    </label>
  </div>
...
.apbd-img-input-ctrn {
  ...
  label {
    ...
    padding:var(--apbd-imgr-in-label-p,10px) !important;
    align-items: center;
    ...
    .apbd-imgr-option-label{
      font-size: var(--apbd-imgr--text-size, 12px);;
    }
    .apbd-imgr-container {
